---
layout: post
title: "e"
author: "raghul-rajasekar"
---
```
n =   
e = 0b1101  
c = 
```

## Solution

An important observation is that `e = 13` and `c` is divisible by 2<sup>13</sup>, hinting that because of the small value of `e` and large value of `n`, there isn't any wrap-around when finding <code>c = m<sup>e</sup> mod n</code>. Thus, we can directly find <code>m = c<sup>1/e</sup></code> to get the flag. I'm not aware of any easy methods to do this in Python as it doesn't support floating-point values of such high precision, so I ended up calculating `m` using binary search:
```python
st = 0
en = c
while st < en: 
	m = (st + en)//2 
	if m**e == c: 
            print(long_to_bytes(m)) 
            break 
        elif m**e < c: 
            st = m + 1 
        else: 
            en = m - 1
```

## Flag

`rgbCTF{lucky_number_13}`

